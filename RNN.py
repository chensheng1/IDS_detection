...
#——————————————————导入数据——————————————————————
f=open('H:\\论文\\毕业论文\\lstm预测实验\\train.csv')  
df=pd.read_csv(f)     

data=np.array(df.iloc[0:500,4])  #获取最高价序列
    ...
    with tf.Session() as sess:
        ...
        for i in range(1000):
            ...
            while(end<len(train_x)):
                ...
                if step%10==0:
                    print(i,step,loss_)
                    print("保存模型：",saver.save(sess,'H:\\论文\\毕业论文\\lstm预测实验\\stock2.model'))
                step+=1
...
def prediction():
	...
	with tf.Session() as sess:
		...
		saver.restore(sess, 'H:\\论文\\毕业论文\\lstm预测实验\\stock2.model') 
		...
		#以折线图表示结果
		ll=np.array(predict)
		data11=np.array(df.iloc[501:601,4])  #获取最高价序列
		print(data11)
		plt.figure()
		plt.figure()
